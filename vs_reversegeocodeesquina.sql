-- FUNCTION: public.vs_reversegeocodeesquina(double precision, double precision, boolean, character)

-- DROP FUNCTION public.vs_reversegeocodeesquina(double precision, double precision, boolean, character);

CREATE OR REPLACE FUNCTION public.vs_reversegeocodeesquina(
...
--Partes que compoe a rua principal
  curStreetParts cursor (street_id bigint, street_name varchar) for
	...
	where line.osm_id in (select sec.id
	  ...
	  and Line.highway in ('primary', 'primary_link','secondary', 'secondary_link','tertiary','tertiary_link', 'residential','service','unclassified')
	  and Line.route is null;

--##########################################
--### VARIAVEIS
--##########################################
  nDistanciaEsquina1 numeric;
  nDistanciaEsquina2 numeric;
  nDistanciaEsquina3 numeric;  
  PontoGPS geometry;
  Ponto geometry;
  PontoOSM geometry;
  PontoGPS2 geometry;
...
begin
  ...
  select pontoNorteref, st_primeraTransversal,pnortereferencia,porientereferencia,osmidprimeratransversal,distanciaEsquina 
    into mipontotransvref,nametrans1, nortereferencia, orientereferencia,lineidtrans1,nDistanciaEsquina1  
     from vs_obtenerTransversal(ponto,pontoosm, StreetParts[1], StreetWays);  
  if not position('(esquina)' in nametrans1)>0 then
    select st_segundaTransversal,osmidsegundatransversal,distanciaEsquina into nametrans2,lineidtrans2,nDistanciaEsquina2 
  ...
  if nametrans2 is null   then
    ...
    --##########################################
        select st_segundaTransversal,osmidsegundatransversal,distanciaEsquina into nametrans2,lineidtrans2,nDistanciaEsquina3 
          from vs_obtenerSegundaTransversal
  ...
  if nametrans2 is null   then
    ...
    --Busca la via recorrida más próxima al punto
    select st_transform(
                        st_closestpoint(
                                        line.way,
                                        ST_CENTROID(line.way)
    ...
    if (nDistanciaEsquina1<=nDistanciaEsquina2) then
	  Ret := Ret || sPrefijo||nametrans1;
    else
      Ret := Ret || sPrefijo||nametrans2;
	end if;    
...
ALTER FUNCTION public.vs_reversegeocodeesquina(double precision, double precision, boolean, character)
    OWNER TO postgres;
