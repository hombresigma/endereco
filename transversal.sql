       ...
       sT_DISTANCE(ST_TRANSFORM(SEC.WAY,4326),
                   ...
                   ST_SetSRID(ST_Point( -75.5690443555856,6.23671525138711),4326)
                  
                  ) as Distancia2,
       sT_DISTANCE(ST_TRANSFORM(st_closestpoint(main.way,sec.way),4326),
                   ST_SetSRID(ST_Point( -75.5690443555856,6.23671525138711),4326)                  
                  ) as DistanciaPuntoAEsquinaTrans,       
	...
	where main.OSM_id = 30659170
       --and sT_DISTANCE(MAIN.WAY,SEC.WAY)=0
	   AND MAIN.OSM_ID<>SEC.OSM_ID
	   AND   ST_DWithin( MAIN.WAY,SEC.WAY,
	                    170
	                   )
	  and sec.highway  in ('primary', 'primary_link','secondary', 'secondary_link','tertiary', 'tertiary_link','residential','unclassified')
