-- FUNCTION: public.vs_reversegeocodepunto(double precision, double precision, boolean, character, integer)

-- DROP FUNCTION public.vs_reversegeocodepunto(double precision, double precision, boolean, character, integer);

CREATE OR REPLACE FUNCTION public.vs_reversegeocodepunto(
...
begin
  ...
  iIdPunto=ipunto;
  if (ipunto<>-1) then 
    select idpunto,st_distance(
                  st_transform(punto,4326),
                  ST_SetSRID(ST_Point( lon,lat),4326)
                 ) as distance into iIdPunto,nada1 from vs_localizacao
    where 
      idpunto=ipunto
                 order by distance asc
                 limit 1;
	...
	if not (nametrans2 is null) then
	  Ret := Ret || ' y '||nametrans2 ; 
        ...
        if not (ret is null or ret='') then
          if iIdPunto<>-1 then
            update vs_localizacao set direccion=ret, lineidtransversal1=lineidtrans1,lineidtransversal2=lineidtrans2 where idpunto=iIdPunto;
          else
...
ALTER FUNCTION public.vs_reversegeocodepunto(double precision, double precision, boolean, character, integer)
    OWNER TO postgres;
